...
void setup()
{
    ...
    myservo2.attach(6);
    setpoint1=pos;
    checkpos(myservo1,pos,magEnc1, myPID1, &input1, &output1, &setpoint1);
    checkpos(myservo2,pos,magEnc2, myPID2, &input2, &output1, &setpoint2);
    tiempo_cp=millis();
...
checkpos(myservo1,pos1,magEnc1, myPID1, &input1, &output1, &setpoint1);
checkpos(myservo2,pos2,magEnc2, myPID2, &input2, &output2, &setpoint2);
...
void checkpos(Servo servo, int p, int magEncIndex, PID myPID, double* input, double* output, double* setpointx){

   tcaselect(magEncIndex);
   *setpointx=p;
   *input = (magnEncGetPosDegrees(magEncIndex));
   myPID.Compute();                  
